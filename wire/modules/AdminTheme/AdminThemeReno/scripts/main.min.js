...
$(document).ready(function() 
{
	...
	// MEMELPOWER HACKS
	// always show all items in left menu
	$("#main-nav a.parent").removeClass('open');
	$("#main-nav a.parent").next('ul').toggle(true);
	$("#main-nav a.parent").removeClass('just-clicked'); 
	
	// clicking on one language tab changes all others too (if visible)
	var clickactive = true; // prevent loop
	$.each(languages, function(index, langid)
	{
		$('body').on('click', '.langTab'+langid, function()
		{
			// prevent loop
			if(clickactive)
			{
				clickactive = false;
				triggerlanguage(langid);
				clickactive = true;
			}
		});
	});
...
function triggerlanguage(langid)
{
	$('[data-lang]').each(function()
	{
		if($(this).data('lang')==langid)
		{
			$(this).trigger('click');
		}
	});
