...
function mp_render_sections($sections, $page_id = null)
{
    ...
    foreach ( $sections as $section) {
        ...
        $headline_formated = $headline? '<h2 class="title is-2 has-text-centered">'.$headline.'</h2><br>' : null;
        $bgimage = $bgimage? 'background-image:url('. $bgimage->size(1680)->url .')' : null;
        $bgcolor = $bgcolor? 'background-color: #'.$bgcolor : null;
        $isvertical = $isvertical? 'is-verticaly-centered' : null;
        ...
        switch ($type){
            ...
            case 'intro':
                ...
                if ($client_get){
                    $client  = '<div class="column is-4 is-label">'.\ProcessWire\__('Client').'</div>';
                    $client .= '<div class="column is-8">'.$client_get.'</div>';
                }
                if ($year_get){
                    $year  = '<div class="column is-4 is-label">'.\ProcessWire\__('Year').'</div>';
                    $year .= '<div class="column is-8">'.$year_get.'</div>';
                }
                if ($devices_get){
                    $devices  = '<div class="column is-4 is-label">'.\ProcessWire\__('Mediums').'</div>';
                ...
                if ($technologies_get){
                    $technologies  = '<div class="column is-4 is-label">'.\ProcessWire\__('Technologies').'</div>';
                    $technologies .= '<div class="column is-8"><a href="'.$pages->get(1111)->url.'">'.$technologies_get.'</a></div>';
                }
                if ($link_get){
                    $link  = '<div class="column is-4 is-label">'.\ProcessWire\__('Visit online').'</div>';
                    ...
                    <div class="hero-body">
                        ...
                        <div class="columns is-verticaly-centered">
                            ...
                            <div class="column is-4-desktop is-4-tablet is-offset-1" id="project-info">
                                <h4 class="title is-4">'.\ProcessWire\__('Project Info').'</h4>
            ...
            case 'grid':
                ...
                if ($image){
                    ...
                    switch ($size){
                        case 'is-half':
                            $image_resized .= $image->size(590)->url;
                            break;
                        case 'is-fullwidth':
                        case null:
                            $image_resized .= $image->size(1200)->url;
                            break;
                    }
                    $image_resized .= '"></div>';
                }
